...
\title{DSC180A Notes: Rank, Non-Negative Rank, and Communication Complexity}
\author{}
\date{}
...
\section{What is Rank?}

\textbf{Definition.}  
The rank of a matrix is the number of linearly independent variables (or columns/rows) in that matrix. Essentially, rank measures the number of variables not accounted for by other variables.
...
The tensor product of two matrices is the projection of one matrix onto another, resulting in an $n \times n'$-dimensional space.
...
\section{Lower Bounds Using Rank}
...
\begin{enumerate}
  \item The smallest number of non-negative rank-1 matrices that sum to $M$.
...
\textbf{Example.}  
The matrix $\begin{pmatrix}1 & 1 & 0 & 0\\ 1 & 0 & 1 & 0\\ 0 & 1 & 0 & 1 \\ 0 & 0 & 1 & 1\end{pmatrix}$ has $\mathrm{rank}(M) = 3$ but $\mathrm{rank}_+(M) = 4$.

\textbf{Lemma.}  
If a matrix is $3 \times 3$ or smaller, $\mathrm{rank}_+(M) = \mathrm{rank}(M)$.

\textbf{Lemma (Fact 2.12).}  
...
\section{Non-Negative Rank Bounds}
...
\begin{theorem}[2.13]
If $M$ is not the all-ones matrix, the communication complexity of $M$ is at least $\log_2 \mathrm{rank}_+(M)$. (Follows from Theorem 2.2.)
\end{theorem}

\begin{lemma}[2.15]
If a Boolean matrix $M$ has a 1-cover of size $r$, then the communication complexity of $M$ is at most $\log_2 r$.
\end{lemma}

\textbf{Remark.}  
These bounds can differ exponentially.

\section{Log-Rank Conjecture}

\begin{conjecture}[2.16]
...
\section{Upper Bounds and Proof Sketches}
...
\section{Properties of Boolean Matrices}
