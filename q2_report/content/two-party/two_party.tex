...
Consider a graph $G = (V,E)$ and subgraphs $G_1, \ldots, G_k \subset G$. Let % We are already discussing multi-party here. This is probably not a good idea, since we start with two party and go from there. Introduce new topics as needed, not all at once, since this will confuse the reader less
$G_i = (V_i, E_i)$ for these subgraphs. For now let us suppose $G$ is
unweighted and undirected. The subgraphs need not cover $G$ and can overlap.
Each player $i$ knows $G$ and $G_i$, but not the other player's graphs. The
goal is, given some starting vertex $v \in V$, and some end vertex $w \in V$
to:
\begin{enumerate}
...
Sometimes, condition \ref{prob:traversal-exists-path} fails, as in Figure \ref{fig:traversal-no-path}. Here, the protocol must output 0.
...
Recall that in two-party $\DISJ$, Alice and Bob are given sets $X, Y \subseteq [n]$ each. The goal is to determine whether $X \cap Y = \emptyset$. We claim $D(\TRAV_2) \geq D(\DISJ)$.
\end{claim}
