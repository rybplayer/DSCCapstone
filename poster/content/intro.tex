...
information possible, so as to avoid one agent dominating the training process.
...
In our model, we consider the learning space as a graph, which each agent will
need to understand properties of in order to find ideal training parameters.
...
Neither of them knows the inputs of the other persons, so they must talk back
and forth until they know enough to compute some function $f(x,y)$


In the worst case, Alice just send all $n$ bits. This is called
