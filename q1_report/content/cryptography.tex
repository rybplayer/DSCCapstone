...
\subsubsectoin{The Millionaire's Problem}
...
This solution has $\Omicron(n \cdot k)$ communication complexity cost, where $n$
...
In the problem start, Alice and Bob each have an input  $x, y \in \R^n$. Alice
...
$\omaga(n)$, where each decision step in the protocol can be modeled with a
gate or simple circuit, we find that there are then $\omaga(n \cdot k)$ total
