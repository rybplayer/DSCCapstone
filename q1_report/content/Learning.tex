...
\subsubsection{Cooperative MARL: Learning with Shared Parameters}
...
\subsubsection{Communication Complexity Result}

Let $d$ = \text{dimension of model parameters}, $H$ = \text{episode horizon}, and $M$ = \text{number of agents}. Then total communication per update can be bounded by:
\[
O(dHM),
\]
...
\newpage
\subsection{Case Study: Learning from Distributed Datasets}
...
Their goal: Find a classifier  H consistent with both  SA and SB, while communicating as little as possible. This models distributed systems, federated learning, and privacy-preserving ML.

\subsubsection{The Realizability Question}
...
\subsubsection{Learning the Classifier}

Once realizability is established, the next step is to learn a classifier. A typical protocol could be:
...
This resembles boosting, where each interaction reduces error. The total communication requirement is
\[
...
This provides a fundamental lower bound: No matter what distributed learning algorithm you design, at least $\Omega(d)$ bits must be exchanged to jointly learn a $d$-dimensional classifier.

You cannot cheat this by clever optimization—communication is inherently tied to dimensionality. This contrasts with MARL, where communication lower bounds depend on:
\begin{itemize}
